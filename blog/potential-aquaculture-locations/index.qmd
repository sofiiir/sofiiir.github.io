...
GitHub repository: [https://github.com/sofiiir/aquaculture-exclusive-economic-zones](https://github.com/sofiiir/aquaculture-exclusive-economic-zones)
...
SST data was acquired from the [NOAA’s 5km Daily Global Satellite Sea Surface Temperature Anomaly v3.1](https://coralreefwatch.noaa.gov/product/5km/index_5km_ssta.php). SST data from 2008 to 2012 are used and averaged for this analysis. This averaging is necessary to account for year-to-year variations in SST. Since this is raster data, we use the `terra` package to read in the data. 
...
## Check CRS
CRS is one of the three components that are essential to check before plotting. CRSs must match for all of the data to accurately perform functions on different data.
...
To reiterate, the three important aspects when working with spatial data are that the extent, resolution, and positions match.
For this analysis, SST and depth need to have the same extent, resolution, and positions. I crop to make sure the extents of the rasters are the same.
...
The total amount of space in the Exclusive Economic Zones (EEZ) on the West Coast that is suitable for oyster aquaculture based on suitable sea surface temperature and suitable bathymetry is `r total_oyst` km^2.
...
I also make polished tables to visualize the differences in suitable aquaculture areas and verify that the map above is accurate. 
```{r}
#| code-fold: true
...
# run the function to identify potentially suitable areas for red sea urchin aquaculture 
...
Engle, C. R., Senten, J. V., Fong Q., Good, M. (July 2025) The economic contribution of aquaculture to the U.S. Western Region. North American Journal of Aquaculture. Volume 87, Issue 3, Pages 155–167. [https://doi.org/10.1093/naaqua/vraf008](https://academic.oup.com/naja/article/87/3/155/8210510)

Flanders Marine Institute (2025): MarineRegions.org. Available online at [www.marineregions.org](https://www.marineregions.org/). Consulted on 2025-11-14.
...
Palomares, M.L.D. and D. Pauly. Editors. 2025. SeaLifeBase. World Wide Web electronic publication. [www.sealifebase.org](https://www.sealifebase.org/search.php), version (04/2025).
